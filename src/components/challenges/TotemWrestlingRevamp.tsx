/**
 * PLAN: Totem Wrestling Revamp
 *
 * Key changes from original:
 *  - Hold & release charge mechanic replaces rapid clicking
 *  - Charge bar has a "peak zone" — releasing inside it multiplies push power
 *  - Charge overshoots the peak and declines if held too long (skill ceiling)
 *  - Opponent has a visible charge bar so players can anticipate their push
 *  - Two separate character sprites that move toward/away from a shared contact point
 *    rather than one composite image sliding across the whole canvas
 *  - Ring-out win condition: push opponent to the edge of a sumo-style arena
 *  - Screen shake scales to push intensity
 *  - Strength stat speeds up charge rate, widens the peak zone, and increases push power
 *  - All interaction is touch/click hold-and-release — no hover or keyboard required
 *
 * New assets needed (drop into /public/challenges/):
 *  - sumo-ring-background.png   — overhead or slight top-down view of an earthen ring
 *                                  with totem carvings around the border
 *  - player-totem-spirit.png    — player's totem spirit facing right
 *  - opponent-totem-spirit.png  — guardian spirit (can reuse player flipped via scaleX(-1))
 *
 * Existing assets that can be reused:
 *  - forest-background.png      — fallback background while new art is being made
 */
...
interface TotemWrestlingRevampProps {
    difficulty?: number;
    strength?: number;
...
const TotemWrestlingRevamp: React.FC<TotemWrestlingRevampProps> = ({
    difficulty = 2,
    strength = 10,
    ...
    useEffect(() => { contactPositionRef.current = contactPosition; }, [contactPosition]);
    ...
    const gameSettings: GameSettings = {
        chargeRate:            1.0 + (strength * 0.10),          // ~1–3.5 pts/frame at 60fps
        peakZoneMin:           Math.max(60, 80 - strength),       // wider peak with more strength
        peakZoneMax:           95,
        declineRate:           2.2,
        basePushPower:         0.04 + (strength * 0.003),        // ~0.04–0.07 per release
        peakPushMultiplier:    1.9,
        opponentBaseInterval:  Math.max(1800, 4200 - (difficulty * 600)),
        timeLimit:             20 - (difficulty * 2),
    };
    ...
    const getOpponentAuraStyle = (): React.CSSProperties => {
        ...
        return {
            ...
            transform: `scale(${1 + t * 0.05}) scaleX(-1)`,
            transition: 'filter 0.08s ease-out, transform 0.08s ease-out',
        };
    };
    ...
    const applyPush = useCallback((power: number, direction: 'player' | 'opponent') => {
        ...
        const impactXPx = contactPositionRef.current * gameAreaWidth;
        spawnImpactParticles(impactXPx);
    ...
    const calculateScore = useCallback((won: boolean): number => {
        ...
        const timeUsed = endTimeRef.current
            ? Math.max(0, (gameSettings.timeLimit * 1000 - (endTimeRef.current - Date.now())) / 1000)
    ...
    const handleGameEnd = useCallback((success: boolean) => {
        if (gameStateRef.current === 'success' || gameStateRef.current === 'failed') return;
        ...
        const scheduleNext = () => {
            ...
            opponentTimerRef.current = setTimeout(() => {
                ...
                opponentIntervalRef.current = window.setInterval(() => {
                    oppCharge = Math.min(100, oppCharge + 2.2);
                    ...
                    if (oppCharge >= 80 + Math.random() * 20) {
                        ...
                        if (gameStateRef.current === 'playing') {
                            const power = gameSettings.basePushPower * (oppCharge / 100) * 1.15;
    ...
    // contactPosition runs 0→1 (player side → opponent side)
    // The two spirits meet at the contact point; they are offset from it by ~16% of width
    const contactXPercent   = contactPosition * 100;
    const playerXPercent    = contactXPercent - 16;   // player sprite left-center
    const opponentXPercent  = contactXPercent + 16;   // opponent sprite left-center

    // Lean angle: losing spirit leans away from contact point
    const playerLean   = Math.min(15, (0.5 - contactPosition) * 40);  // leans right if losing
    const opponentLean = Math.min(15, (contactPosition - 0.5) * 40);  // leans left if losing
    ...
    return (
        <div ref={containerRef} className="w-full max-w-2xl mx-auto">
            ...
            >
                ...
                {/* TODO: replace src with '/challenges/sumo-ring-background.png' */}
                <img
                ...
                <div
                    ...
                    style={{
                        left:      `${playerXPercent}%`,
                        bottom:    '8%',
                        transform: `translateX(-50%) rotate(${playerLean}deg)`,
                        width:     '18%',
                        maxWidth:  '90px',
                        ...getAuraStyle(),
                    }}
                >
                    {/* TODO: replace with '/challenges/player-totem-spirit.png' */}
                    <img
                ...
                {/* Opponent spirit (mirrored via scaleX(-1)) */}
                <div
                    className="absolute pointer-events-none"
                    style={{
                        ...
                        // getOpponentAuraStyle already includes scaleX(-1) when charging;
                        // always flip when idle
                        transform: opponentCharge === 0
                            ? `translateX(-50%) scaleX(-1) rotate(${-opponentLean}deg)`
                            : undefined,
                    }}
                >
                    {/* TODO: replace with '/challenges/opponent-totem-spirit.png' */}
                    <img
                        src="/challenges/bearwolfstandoff.png"
                        alt="Guardian spirit"
                        className="w-full h-auto"
                ...
                <div className="absolute top-3 left-4 right-4 h-2.5 bg-gray-900/70 rounded-full overflow-hidden border border-gray-600">
                    ...
                    <div
                        className="absolute top-0 left-0 h-full bg-blue-500 transition-all duration-200 rounded-full"
                        style={{ width: `${contactPosition * 100}%` }}
                    />
                    {/* Opponent (red) fills from right */}
                    <div
                        className="absolute top-0 right-0 h-full bg-red-500 transition-all duration-200 rounded-full"
                        style={{ width: `${(1 - contactPosition) * 100}%` }}
                    />
                    {/* Center marker */}
...
export default TotemWrestlingRevamp;
