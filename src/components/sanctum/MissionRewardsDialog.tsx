...
import { Swords, Award, Sparkles, Star, X } from 'lucide-react';
import { RuneIcon } from '../RunesDisplay';

interface MissionRewardsDialogProps {
  missionName: string;
  xpGained: number;
  runesGained: {
    lesser: number;
...
const MissionRewardsDialog: React.FC<MissionRewardsDialogProps> = ({
  missionName,
  xpGained,
  runesGained,
  onClose,
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[70] flex items-start sm:items-center justify-center pt-4 sm:pt-0 overflow-y-auto">
      <div className="relative w-full max-w-lg transform transition-all bg-gradient-to-br from-indigo-300/40 to-purple-300/40 dark:from-indigo-700/40 dark:to-purple-700/40 rounded-xl border-2 border-indigo-400 dark:border-indigo-600 p-1 animate-fade-in scale-100 mx-4 mb-24 sm:mb-0">
        <div className="absolute -inset-1">
          <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-xl blur opacity-30 animate-pulse"></div>
        </div>
...
export default MissionRewardsDialog;
